...
function calculateTickInterval(businessDelta, screenDelta, tickInterval, forceTickInterval, axisDivisionFactor, multipliers, allowDecimals, addTickCount) {
    var interval = getIntervalByFactor(businessDelta, screenDelta, axisDivisionFactor, addTickCount),
        result = 1,
        onlyIntegers = false === allowDecimals;
...
function calculateTickIntervalDateTime(businessDelta, screenDelta, tickInterval, forceTickInterval, axisDivisionFactor, multipliers, allowDecimals, addTickCount, gapSize) {
    var result, interval = getIntervalByFactor(businessDelta, screenDelta, axisDivisionFactor);
    if (!forceTickInterval || !tickInterval) {
...
function getTickIntervalByCustomTicks(getValue, postProcess) {
    return function(ticks) {
        ...
        return postProcess(mathAbs(adjust(getValue(ticks[1]) - getValue(ticks[0])))) || void 0
    }
}
...
function generator(options, getBusinessDelta, calculateTickInterval, calculateMinorTickInterval, getMajorTickIntervalByCustomTicks, getMinorTickIntervalByCustomTicks, convertTickInterval, calculateTicks, calculateMinorTicks, processScaleBreaks) {
    ...
    function generateMajorTicks(ticks, data, businessDelta, screenDelta, tickInterval, forceTickInterval, customTicks, breaks) {
        if (customTicks.majors && !options.showCalculatedTicks) {
            ticks.breaks = breaks;
            return ticks
        }
        var majorTicks, gaps = breaks.filter(function(b) {
            return b.gapSize
        });
        tickInterval = correctUserTickInterval(tickInterval, businessDelta, screenDelta);
        tickInterval = calculateTickInterval(businessDelta, screenDelta, tickInterval, forceTickInterval, options.axisDivisionFactor, options.numberMultipliers, options.allowDecimals, breaks.length, gaps[0] && gaps[0].gapSize.days);
    ...
    function generateMinorTicks(ticks, data, businessDelta, screenDelta, minorTickInterval, minorTickCount, customTicks) {
        ...
        if (customTicks.minors && !options.showMinorCalculatedTicks) {
            return ticks
        }
...
function logarithmicGenerator(options) {
    ...
    return generator(options, getBusinessDeltaLog(base), calculateTickIntervalLog, calculateMinorTickInterval, getTickIntervalByCustomTicks(log, getValue), getTickIntervalByCustomTicks(getValue, getValue), getValue, calculateTicks(addIntervalLog(base), correctMinValueByEndOnTick(floor, ceil, resolveEndOnTickLog(base), options.endOnTick), adjustIntervalLog, resolveEndOnTickLog(base), resolveExtraTickForHiddenDataPointLog(base)), calculateMinorTicks(updateTickInterval, addInterval, floor, ceilNumber, ceil), getScaleBreaksProcessor(getValue, log, function(value, correction) {
...
function dateGenerator(options) {
    function floor(value, interval) {
        ...
        if ("years" in intervalObject) {
            ...
            if ("quarters" in intervalObject) {} else {
                if ("months" in intervalObject) {
                    value.setMonth(floorNumber(value.getMonth(), intervalObject.months, 0))
                } else {
                    if ("weeks" in intervalObject) {} else {
                        if ("days" in intervalObject) {} else {
