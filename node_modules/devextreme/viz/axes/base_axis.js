...
function createMajorTick(axis, renderer, skippedCategory) {
    ...
    return tick(axis, renderer, options.tick, options.grid, skippedCategory, axis._translator.getBusinessRange().stubData)
}
...
Axis.prototype = {
    ...
    _createAxisGroups: function() {
        ...
        that._axisBreaksGroup = renderer.g().attr({
            "class": classSelector + "breaks"
        })
    },
    ...
    dispose: function() {
        var that = this;
        [that._axisElementsGroup, that._axisStripGroup, that._axisGroup, that._axisBreaksGroup].forEach(function(g) {
            g.dispose()
        });
    ...
    calculateInterval: function(value, prevValue) {
        var options = this._options;
        return !options || options.type !== constants.logarithmic ? _abs(value - prevValue) : vizUtils.getLog(value / prevValue, options.logarithmBase)
    },
        ...
        var that = this,
            validateBusinessRange = function(range, min, max) {
                function validate(valueSelector, baseValueSelector, optionValue) {
                    range[valueSelector] = isDefined(optionValue) ? optionValue : isDefined(range[valueSelector]) ? range[valueSelector] : range[baseValueSelector]
                }
    ...
    _createTicksAndLabelFormat: function(range, incidentOccurred) {
        ...
        if (options.type === constants.discrete && "datetime" === options.dataType && !this._hasLabelFormat && ticks.ticks.length) {
    ...
    _reinitTranslator: function(range) {
        ...
        if (that._options.type !== constants.discrete) {
            if (length && !that._options.skipViewportExtending && (!isDefined(that._zoomArgs) || !that.isArgumentAxis)) {
    ...
    _applyMargins: function(range) {
        var marginSizeMultiplier, that = this,
            ...
            marginValue = 0,
            type = options.type,
            valueMarginsEnabled = options.valueMarginsEnabled && type !== constants.discrete && "semidiscrete" !== type,
        ...
        function addMargin(value, margin, marginOption) {
            if (!isDefined(marginOption) && !(margins.percentStick && 1 === _abs(value) && !isArgumentAxis)) {
